...
export default function InvoiceHistory({ invoices, onView, onDelete, onClose }: Props) {
        ...
        <div className="mx-auto max-w-4xl space-y-3">
          {sorted.length === 0 ? (
            ...
            sorted.map(inv => (
              <div
                key={inv.id}
                className="flex items-center gap-4 rounded-xl bg-white p-4 shadow transition hover:shadow-md"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500">
                    {formatDate(inv.invoiceDate)} • {inv.items.length} поз. • {formatNumber(inv.totalAmount)} руб.
                  </p>
                </div>
